...
class SimpleDB(object):
    ...
    def __init__(self, aws_access_key, aws_secret_access_key, db='sdb.amazonaws.com', 
        ...
        self.http_async = tornado.httpclient.AsyncHTTPClient()
        self.encoder = encoder

    def _make_request(self, request):
        logging.error( 'SYNCHRONOUS SDB REQUEST! BAD!!' )
    ...
    def _on_async_request_response(self, user_callback, tornado_response):
        ...
        if user_callback:
            ...
            resp.tornado_response = tornado_response
            return user_callback( resp )

    def _make_request_async(self, request, user_callback=None):
        headers = {'Content-Type': 'application/x-www-form-urlencoded; charset=utf-8',
#                   'Connection':'close',
                   'host': self.db}
        request.set_parameter('Version', self.service_version)
    ...
    def delete_attributes(self, domain, item, attributes=None, async=True, callback=None):
        """
    ...
    def get_attributes_async(self, callback, domain, item, attributes=None):
        if isinstance(domain, Domain):
            domain = domain.name
        if isinstance(item, Item):
            item = item.name
        ...
        def cb(response, callback, domain, item, attrs, start_time):
            response.amazon_response_time = time.time() - start_time
            if response.content:
                e = ET.fromstring(response.content)
                attr_node = e.find('{%s}GetAttributesResult' % self.ns)
            else:
            ...
            #print 'amazon request id %s in %.2f seconds' % (response.request_id, amazon_response_time)
            attrs = dict.fromkeys(attrs or [])
            if attr_node:
                attrs.update(self._parse_attributes(domain, attr_node))
            return callback(attrs, response)
    ...
    def _select_single_async(self, domain, expression, user_callback):
            ...
            if response.content:
                ...
                if item_node is not None:
                    for item in item_node.findall('{%s}Item' % self.ns):
                        ...
                        items.append( Item(self, domain, name, attributes) )
                return user_callback(items)
            else:
                return user_callback(None)

        self._make_request_async(request, got_response)
    ...
    def _got_select_async_response(self, request, domain, callback, response):
        items = []
        e = ET.fromstring(response.content)
        item_node = e.find('{%s}SelectResult' % self.ns)
        if item_node is not None:
            ...
            next_token = item_node.find('{%s}NextToken' % self.ns)
            if next_token is None:
                return callback( items, None)
            else:
                request.parameters['NextToken'] = next_token.text
                stream_callback = functools.partial(self._got_select_async_response, request, domain, callback)
                callback( items, functools.partial(self._make_request_async, request, stream_callback) )
        
    def _select_async(self, domain, expression, callback):
        if not isinstance(domain, Domain):
            domain = Domain(domain, self)
        data = {
            'Action': 'Select',
        ...
        request = Request("POST", self._sdb_url(), data)
        stream_callback = functools.partial(self._got_select_async_response, request, domain, callback)
        self._make_request_async(request, stream_callback)
...
class Item(DictMixin):
    @classmethod
    def load(cls, simpledb, domain, name, user_callback = None):
        if user_callback:
            def cb(cls, simpledb, domain, name, user_callback, attrs, response):
...
#SDB = SimpleDB(config['sdb_access_key'], config['sdb_access_secret'], db=config['sdb_server'], secure = config['sdb_secure'])
